...
from agents.DRO_Wasserstein_experts import Optimization_Modeling, Code_Generating, Transfer_Modeling
...
def pipline(result_dir: str, theta: str, problem: Dict, data_discription: Dict, train_dir: str, test_dir: str, model: str, temperature: float) -> None:
    ...
    mean = calculate_mean(history_data)
    #------------------------------------------------------------------------------------------------------------------------
    ...
    if os.path.exists(code_path):
        ...
        code_response = code_generating.run(problem, data_discription, row, theta, mean, initial_model, math_model, train_dir, test_dir, result_dir)
